...
# Случайная экипировка для классов
CLASS_EQUIPMENT = {
    "Воин": [
        "Кольчуга, длинный меч, щит",
        "Кожаный доспех, длинный меч, два кинжала",
        "Кольчуга, боевой молот, арбалет",
        "Кольчуга, секира, длинный лук",
        "Кольчуга, двуручный меч",
    ],
    "Паладин": [
        "Кольчуга, длинный меч, щит, священный символ",
        "Кольчуга, боевой молот, щит, священный символ",
        "Кольчуга, длинное копьё, щит, священный символ",
        "Кольчуга, длинный меч, арбалет, священный символ",
    ],
    "Плут": [
        "Кожаный доспех, короткий меч, два кинжала, воровские инструменты",
        "Кожаный доспех, рапира, лёгкий арбалет, воровские инструменты",
        "Кожаный доспех, короткий меч, короткий лук, воровские инструменты",
        "Кожаный доспех, два кинжала, короткий лук, воровские инструменты",
    ],
    "Волшебник": [
        "Посох, компонентная сумка, книга заклинаний",
        "Кинжал, компонентная сумка, книга заклинаний",
        "Посох, арбалет, компонентная сумка, книга заклинаний",
        "Посох, компонентная сумка, книга заклинаний, учёные инструменты",
    ],
    "Жрец": [
        "Кольчуга, щит, булава, священный символ",
        "Кольчуга, щит, боевой молот, священный символ",
        "Кожаный доспех, булава, священный символ",
        "Кольчуга, булава, арбалет, священный символ",
    ],
    "Бард": [
        "Кожаный доспех, рапира, лютня, арбалет",
        "Кожаный доспех, длинный меч, флейта, короткий лук",
        "Кожаный доспех, рапира, скрипка, арбалет",
        "Кожаный доспех, короткий меч, лютня, короткий лук",
    ],
    "Варвар": [
        "Кожаный доспех, большой топор, два ручных топора",
        "Без доспеха, большой меч, два копья",
        "Кожаный доспех, боевой молот, два ручных топора",
        "Без доспеха, большой топор, длинный лук",
    ],
    "Друид": [
        "Кожаный доспех, булава, щит, друидический фокус",
        "Кожаный доспех, булава, друидический фокус, посох",
        "Кожаный доспех, серп, щит, друидический фокус",
        "Кожаный доспех, булава, арбалет, друидический фокус",
    ],
    "Монах": [
        "Короткий меч, 10 дротиков",
        "Простое оружие ближнего боя, 10 дротиков",
        "Короткий меч, короткий лук",
        "Простое оружие ближнего боя, арбалет",
    ],
    "Следопыт": [
        "Кожаный доспех, длинный меч, длинный лук",
        "Кожаный доспех, два коротких меча, длинный лук",
        "Кожаный доспех, копьё, длинный лук",
        "Кожаный доспех, длинный меч, арбалет",
    ],
    "Чародей": [
        "Лёгкий арбалет, компонентная сумка, волшебный фокус",
        "Два кинжала, компонентная сумка, волшебный фокус",
        "Лёгкий арбалет, посох, компонентная сумка, волшебный фокус",
        "Кинжал, компонентная сумка, волшебный фокус",
    ],
    "Изобретатель": [
        "Кожаный доспех, два кинжала, инструменты ремесленника",
        "Кожаный доспех, лёгкий арбалет, инструменты ремесленника",
        "Кольчуга, булава, инструменты ремесленника",
        "Кожаный доспех, посох, инструменты ремесленника",
    ],
}

def generate_random_equipment(char_class: str) -> str:
    """Генерирует случайную экипировку для класса"""
    equipment_list = CLASS_EQUIPMENT.get(char_class, ["Базовая экипировка"])
...
def make_menu_inline_keyboard():
    """Inline-клавиатура для меню со всеми командами"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="📚 Энциклопедия", callback_data="menu_encyclopedia")
            ],
            [
                InlineKeyboardButton(text="⏸️ Короткий отдых", callback_data="menu_short_rest"),
                InlineKeyboardButton(text="🛌 Долгий отдых", callback_data="menu_long_rest")
            ],
            [
                InlineKeyboardButton(text="📰 Слухи", callback_data="menu_rumors"),
                InlineKeyboardButton(text="💾 Сохранение", callback_data="menu_save")
            ],
            [
...
async def set_class(message: Message, state: FSMContext):
    ...
    # Генерируем случайную экипировку для класса
    equipment = generate_random_equipment(cl)
    await state.update_data(char_class=cl, equipment=equipment)
...
async def finish_creation(message: Message, state: FSMContext):
    ...
    character_info += (
        f"\n<b>Снаряжение:</b>\n\n"
        f"🎒 {data.get('equipment', 'Базовая экипировка для класса')}\n\n"
        f"💰 Монеты: {coins_amount} золотых\n\n"
        f"<b>Счётчик дней:</b>\n\n"
        f"📅 День {data.get('day_counter', 0)}\n\n"
...
async def menu_status_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await process_user_turn(callback.message, state, "статус")
...
async def menu_inventory_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await process_user_turn(callback.message, state, "инвентарь")
...
async def menu_spells_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await process_user_turn(callback.message, state, "заклинания")

@router.callback_query(F.data == "menu_reputation")
async def menu_reputation_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Репутация в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "репутация")

@router.callback_query(F.data == "menu_quests")
async def menu_quests_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Задания в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "задания")

@router.callback_query(F.data == "menu_encyclopedia")
async def menu_encyclopedia_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Энциклопедия в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "энциклопедия")

@router.callback_query(F.data == "menu_short_rest")
async def menu_short_rest_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Короткий отдых в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "короткий отдых")

@router.callback_query(F.data == "menu_long_rest")
async def menu_long_rest_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Долгий отдых в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "долгий отдых")

@router.callback_query(F.data == "menu_rumors")
async def menu_rumors_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Слухи в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "слухи")

@router.callback_query(F.data == "menu_save")
async def menu_save_callback(callback: types.CallbackQuery, state: FSMContext):
    """Обработчик кнопки Сохранение в меню"""
    await callback.answer()
    await process_user_turn(callback.message, state, "сохранение")
...
async def menu_trade_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    await process_user_turn(callback.message, state, "торговля")
